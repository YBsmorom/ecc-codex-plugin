...
- The active `hooks/hooks.json` is Codex-safe and contains no `async` declarations. Former async
  entries are adapted into bounded Codex hook entries. The preserved Claude Code source graph lives
  at `docs/upstream/claude-code-hooks.json`.
